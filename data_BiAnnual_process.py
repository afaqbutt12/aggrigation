...
def process_BiAnnual_data(company_id, internal_code_id, year, start_month, site_code):
    ...
    # print("Total Year :: ", data_min_year)
...
# def process_BiAnnual_data(company_code, year):
#     MONGODB_URL = os.getenv("MONGODB_URL", "mongodb://localhost:27017/")
#     MONGODB_DB_NAME = os.getenv("MONGODB_DB_NAME", "ensogov")
...
#     # Define the query to retrieve data
    ...
    #         {"$and": [
    #             {"type_year": min_year},
    #             {"semi_annual": {"$exists": True, "$ne": None}}  # Condition for quarter field existence and not null
    #         ]},
    #         {"type_year": {"$gt": min_year}, "semi_annual": {"$exists": True, "$ne": None}}
    #     ]
    # }
...
#     query = {
#             "company_code": company_code,
#             "semi_annual": {"$exists": True, "$ne": None}
#         }


#     result = list(cdata_BiAnnual_collection.find(query)) 

#     process_yearly_data(result, company_code)
